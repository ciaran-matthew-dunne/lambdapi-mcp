...
def test_goals_outside_proof_is_empty(lsp, fixture_path, stdlib):
    if not stdlib:
        pytest.skip("Stdlib required for proof.lp")
...
def test_goals_inside_proof(lsp, fixture_path, stdlib):
    if not stdlib:
        pytest.skip("Stdlib required for proof.lp")
